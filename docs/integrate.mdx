  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const params = await sbMacro.getParams(
        torexAddr,
        flowRateBN,
        ethers.ZeroAddress, // distributor: deprecated, always pass zero
...
4. Set `referrer` to `ethers.ZeroAddress` if you don't want to specify one.
...
:::note `distributor` parameter is deprecated
The `distributor` parameter in `SBMacro.getParams` is no longer supported and will be removed in a future release. The on-chain distribution-fee module was removed in SuperBoring contracts v2.0.0 (the full TOREX fee now goes to stakers). For now, always pass `ethers.ZeroAddress` as the distributor argument.
:::
